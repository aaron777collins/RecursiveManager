...
export function registerDebugCommand(program: Command): void {
  program
    ...
    .action(async (agentId: string, options: { logs?: boolean; state?: boolean; tasks?: boolean; all?: boolean }) => {
      try {
        ...
        if (showAll || options.logs) {
          ...
          console.log();
          console.log(info('View full logs at: ') + code('~/.recursive-manager/logs/agent-' + agentId + '.log'));
          console.log();
        }

        console.log(info('Use ' + code('--logs') + ', ' + code('--state') + ', or ' + code('--tasks') + ' to filter debug information'));
        console.log();
